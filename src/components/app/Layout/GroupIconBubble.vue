<template>
  <div class="relative-position q-px-sm q-pl-md">
    <NotificationBall :value="unread" :size="ballSize"/>
    <!-- <div class=" round cursor-pointer bubble"> -->
    <q-img class="full-width round" ratio="1" :src="imgURL" @mouseover="isHovered = true" @mouseleave="isHovered = false">
      <q-tooltip class="bg-blue-grey-10 text-white " max-width="14em" anchor="center right" self="center start" transition-show="jump-right" transition-hide="jump-left">
        <div class="ellipsis-2-lines text-size-1-5">
          {{ name }}
        </div>
      </q-tooltip>
    </q-img>
...
import NotificationBall from './misc/AlertBall.vue';
...
const props = withDefaults(defineProps<GroupChatBubbleProps>(), {
...
const ballSize = computed(() => {
  if (isHovered.value) return 'lg'
  if (props.unread) return 'sm'
  return ''
})
</script>
